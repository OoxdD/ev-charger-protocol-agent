"""SM4 分组密码（ECB），用于南网业务载荷软加密。

纯 Python 实现，无第三方依赖。密钥/分组均为 16 字节。
"""

from __future__ import annotations

_SBOX = [
    ,
]

_FK = [0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC]

_CK = [
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269, 0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249, 0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229, 0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209, 0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279,
]


def _rotl(x: int, n: int) -> int:
    return ((x << n) | (x >> (32 - n))) & 0xFFFFFFFF


def _tau(a: int) -> int:
    return (
        (_SBOX[(a >> 24) & 0xFF] << 24)
        | (_SBOX[(a >> 16) & 0xFF] << 16)
        | (_SBOX[(a >> 8) & 0xFF] << 8)
        | _SBOX[a & 0xFF]
    )


def _l(b: int) -> int:
    return b ^ _rotl(b, 2) ^ _rotl(b, 10) ^ _rotl(b, 18) ^ _rotl(b, 24)


def _l_prime(b: int) -> int:
    return b ^ _rotl(b, 13) ^ _rotl(b, 23)


def _key_expand(key: bytes) -> list[int]:
    if len(key) != 16:
        raise ValueError("SM4 密钥必须为 16 字节")
    mk = [int.from_bytes(key[i : i + 4], "big") for i in range(0, 16, 4)]
    k = [mk[i] ^ _FK[i] for i in range(4)]
    rk: list[int] = []
    for i in range(32):
        # T'(k[i+1]⊕k[i+2]⊕k[i+3]⊕CK[i])，注意不含 k[i]
        t = _l_prime(_tau(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ _CK[i]))
        k.append(k[i] ^ t)
        rk.append(k[i + 4])
    return rk


def _crypt_block(block: bytes, rk: list[int], decrypt: bool = False) -> bytes:
    if len(block) != 16:
        raise ValueError("SM4 分组必须为 16 字节")
    x = [int.from_bytes(block[i : i + 4], "big") for i in range(0, 16, 4)]
    keys = list(reversed(rk)) if decrypt else rk
    for i in range(32):
        t = _l(_tau(x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ keys[i]))
        x.append(x[i] ^ t)
    out = x[35].to_bytes(4, "big") + x[34].to_bytes(4, "big") + x[33].to_bytes(4, "big") + x[32].to_bytes(4, "big")
    return out


def sm4_ecb_encrypt(key: bytes, data: bytes) -> bytes:
    """ECB 加密；不足 16 字节时后填 0x00（南网附录 B.1）。"""
    if len(data) % 16:
        data = data + b"\x00" * (16 - (len(data) % 16))
    rk = _key_expand(key)
    out = bytearray()
    for i in range(0, len(data), 16):
        out.extend(_crypt_block(data[i : i + 16], rk, decrypt=False))
    return bytes(out)


def sm4_ecb_decrypt(key: bytes, data: bytes) -> bytes:
    """ECB 解密；密文长度须为 16 的整数倍。不自动去填充。"""
    if not data or len(data) % 16:
        raise ValueError("SM4 密文长度须为 16 的整数倍")
    rk = _key_expand(key)
    out = bytearray()
    for i in range(0, len(data), 16):
        out.extend(_crypt_block(data[i : i + 16], rk, decrypt=True))
    return bytes(out)
